...
export function ItemDetailDialog({ item, open, onOpenChange }: ItemDetailDialogProps) {
  ...
  const handleDelete = async () => {
    try {
      ...
      onOpenChange(false);
      toast.success('单品已删除', {
        description: item.name ? `"${item.name}" 已移除。` : '单品已从你的衣橱中移除。',
      });
    } catch (error) {
                        ...
                        <span>
                          ...
                          {item.last_worn_at && (
                            <span className="text-muted-foreground">
                              {' '}• 上次：{new Date(item.last_worn_at).toLocaleDateString('zh-CN')}
                            </span>
                          )}
                        </span>
                      </div>
                  ...
                  <div className="space-y-2 pt-2 border-t">
                    ...
                    <div className="space-y-1.5">
                      <div className="flex justify-between text-xs text-muted-foreground">
                        <span>清洗后穿着次数：{item.wears_since_wash}/{item.effective_wash_interval}</span>
                        {item.needs_wash && (
                      ...
                      {item.last_washed_at && (
                        <p className="text-xs text-muted-foreground">
                          上次清洗：{new Date(item.last_washed_at).toLocaleDateString('zh-CN')}
                        </p>
                      )}
                    </div>
                        ...
                        <CollapsibleContent className="mt-1.5 space-y-1">
                          {washHistory.map((wash) => (
                            <div key={wash.id} className="text-xs text-muted-foreground flex items-center gap-2">
                              <span>{new Date(wash.washed_at).toLocaleDateString('zh-CN')}</span>
                    ...
                    <div className="space-y-2 pt-2 border-t">
                      ...
                      {Object.keys(wearStats.wear_by_month).length > 0 && (
                        <div className="space-y-1">
                          <p className="text-xs text-muted-foreground">近6个月</p>
                          <div className="flex items-end gap-1 h-12">
                            {Object.entries(wearStats.wear_by_month).map(([month, count]) => {
                              ...
                              return (
                                <div key={month} className="flex-1 flex flex-col items-center gap-0.5" title={`${month}: ${count}次穿着`}>
                                  <div
                            ...
                            {wearHistory.map((entry) => (
                              <div key={entry.id} className="text-xs flex items-start gap-2">
                                <span className="text-muted-foreground whitespace-nowrap">
                                  {new Date(entry.worn_at).toLocaleDateString('zh-CN')}
                                </span>
                                {entry.occasion && (
                      ...
                      <div className="flex items-center gap-2 text-sm font-medium">
                        ...
                        {item.tags?.logprobs_confidence != null && (
                          <Badge variant="outline" className="text-xs">
                            {Math.round(item.tags.logprobs_confidence * 100)}% 置信度
                          </Badge>
                        )}
                      </div>
                      {item.ai_description && (
